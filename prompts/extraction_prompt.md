...
- **Preserve figures and wording exactly as written.** Keep numbers, currencies, percentages and dates in their original form — `$1.4M (flat since last year)` stays that way, never `1400000`. A later step checks the briefing's citations against these exact strings, so a paraphrased figure breaks that check.
